...
TEST_CASE("reports error when soc jumps abruptly") {
  ...
  REQUIRE(validateSOCreadings(socReadings, numOfSocReadings, SOCMaxDeltaValue) == 0);
}
...
TEST_CASE("reports error when current jumps abruptly") {
  ...
  REQUIRE(validateSOCreadings(currentReadings, numOfCurReadings, CurrentMaxDeltaValue) == 0);
}
